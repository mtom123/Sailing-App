...
self.addEventListener('activate', (event) => {
  // Keep ONLY current version caches — delete EVERYTHING else (v6, v2.2, v2.3, etc.)
  ...
  event.waitUntil(
    caches
      ...
      .then((keys) => {
        ...
        console.log('[SW v3.0] Deleting ALL old caches:', toDelete)
        return Promise.all(toDelete.map((k) => caches.delete(k)))
      })
      .then(() => {
        // Also unregister old service workers (different scriptURL)
      ...
      .then((clients) => {
        for (const c of clients) {
          ...
          c.navigate(`${c.url.split('?')[0]}?v=${Date.now()}`)
        }
      })
  );
});
